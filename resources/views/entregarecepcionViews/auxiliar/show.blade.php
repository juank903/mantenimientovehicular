    ...
    <div id="imprimible">
        ...
        <div class="imprimible px-4 py-1 sm:px-6">
            <h2 class="imprimible text-xl leading-8 font-medium text-gray-900">
                Entrega Recepción de vehículo - solicitud No {{ $vehiculo['id'] }}<br />
            </h2>
    ...
    @push('scripts')
        <script>
            ...
            btnCancelar.addEventListener('click', () => {
                modalConfirmacion.classList.add('hidden'); // Ocultar el modal
            });

            btnConfirmar.addEventListener('click', () => {
                modalConfirmacion.classList.add('hidden'); // Ocultar el modal después de confirmar

                const asignacionId = "{{ $asignacion['id'] }}";
                console.log(asignacionId);

                fetch(`/api/entregarvehiculo/policia?asignacion_id=${asignacionId}`, { // Ruta de la API
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                                'content') // Si usas CSRF
                        },
                    })
                    ...
                    .catch(error => {
                        ...
                        alert('Ocurrió un error al entregar el vehículo.');
                    });
            });
        </script>
    @endpush
